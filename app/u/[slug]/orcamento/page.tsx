  ...
  return (
    ...
    >
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mt-8 lg:grid lg:grid-cols-[minmax(0,2fr)_minmax(0,3fr)] lg:items-start lg:gap-16">
          {/* ── Coluna esquerda: info + imagem ── */}
          <div>
                ...
                <div className="p-4">
                  ...
                  {selectedService.basePrice ? (
                    <p className="mt-3 font-fraunces text-lg font-bold text-ink">
                      {selectedService.pricingType === "FIXED"
                        ? formatMoney(selectedService.basePrice.toString())
                        : `A partir de ${formatMoney(selectedService.basePrice.toString())}`}
                    </p>
                  ) : null}
                </div>
              </div>
          ...
          <div className="mt-8 lg:mt-0">
            {query.success ? (
              <div
                aria-live="polite"
                className="rounded-xl border border-mint bg-mint/40 p-6"
              >
                <p className="font-fraunces text-xl font-bold text-leaf">
                  Solicitação enviada!
                </p>
                <p className="mt-2 text-sm leading-6 text-ink-muted">
                  Sua solicitação foi registrada. O negócio avaliará as informações e
